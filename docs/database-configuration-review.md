...
Docker Compose 已增加 PostgreSQL/pgvector 服务，但后端主业务仍然默认使用 SQLite；这是为后续 PostgreSQL 表结构迁移和 pgvector 向量检索做准备。

项目已声明 `psycopg[binary]` 作为 PostgreSQL Python 驱动依赖。

项目已新增 PostgreSQL 连接健康检查服务，用于验证 PostgreSQL URL 能否执行最小查询 `SELECT 1`。本地已经通过 `DATABASE_URL=postgresql://...` 验证后端代码可以连上 Docker Compose 中的 PostgreSQL。

在本轮数据库配置整理完成时，PostgreSQL 还没有接入主业务链路。也就是说，当时项目已经具备 PostgreSQL 连接验收能力，但文档、chunks、embeddings、conversations 等业务数据暂时仍然走 SQLite。

## 后续阶段更新：PostgreSQL / pgvector 检索已接入

后续开发已经把 PostgreSQL / pgvector 从“连接验收能力”推进到了“可选知识库检索后端”。

当前状态是：

- PostgreSQL 已经可以存储知识库 documents、chunks 和 chunk embeddings。
- LangGraph Agent 可以通过 `retriever_backend=postgresql` 使用 PostgreSQL / pgvector 检索。
- 带会话保存的 LangGraph Agent 也可以使用 PostgreSQL 检索。
- 会话、消息、摘要和反馈仍然保留在 SQLite。

因此，项目当前不是“SQLite 已经被 PostgreSQL 完全替换”，而是：

```text
PostgreSQL / pgvector：负责实验性知识库语义检索
SQLite：继续负责默认学习数据、会话和反馈等轻量状态
```

更完整的阶段边界记录见：

```text
docs/postgresql-stage-review.md
```
