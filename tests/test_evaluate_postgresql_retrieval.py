from week10.evaluate_postgresql_retrieval import (
    evaluate_postgresql_unknown_questions,
...
def test_evaluate_postgresql_unknown_questions(monkeypatch):
    captured_questions = []
    ...
    result = evaluate_postgresql_unknown_questions(
        FakeConnection(),
        questions=[
            "公司有没有股票期权？",
            "公司食堂几点开门？",
...
def test_evaluate_postgresql_unknown_questions_records_risky_recall(
    monkeypatch,
):
    def fake_search_postgresql_chunks_by_question(
        connection,
    ...
    assert result["items"][0] == {
        ...
        "passed": False,
        "result_count": 1,
        "top_result_text": "员工每天需要完成 8 小时工作。",
    }
